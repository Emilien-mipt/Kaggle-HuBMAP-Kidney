"""
get_tp_fp_fn, SoftDiceLoss, and DC_and_CE/TopK_loss are from
https://github.com/MIC-DKFZ/nnUNet/blob/master/nnunet/training/loss_functions
"""
...
class GDiceLoss(nn.Module):
    ...
    def forward(self, net_output, gt):
        ...
        divided: torch.Tensor = (
            -2 * (einsum("bc->b", intersection) + self.smooth) / (einsum("bc->b", union) + self.smooth)
        )
        gdc = divided.mean()
...
class GDiceLossV2(nn.Module):
    def __init__(self, apply_nonlin=None, smooth=1e-5):
        ...
        Generalized Dice;
        Copy from:
        https://github.com/wolny/pytorch-3dunet/blob/6e5a24b6438f8c631289c10638a17dea14d42051/unet3d/losses.py#L75
        paper: https://arxiv.org/pdf/1707.03237.pdf
        tf code: https://github.com/NifTK/NiftyNet/blob/dev/niftynet/layer/loss_segmentation.py#L279
        """
        super().__init__()
    ...
    def forward(self, net_output, gt):
        ...
        class_weights = Variable(1.0 / (target_sum * target_sum).clamp(min=self.smooth), requires_grad=False)
...
class SSLoss(nn.Module):
    def __init__(self, apply_nonlin=None, batch_dice=False, do_bg=True, smooth=1.0, square=False):
        """
        Sensitivity-Specifity loss
        paper: http://www.rogertam.ca/Brosch_MICCAI_2015.pdf
        tf code:
        https://github.com/NifTK/NiftyNet/blob/df0f86733357fdc92bbc191c8fec0dcf49aa5499/\
        niftynet/layer/loss_segmentation.py#L392
        """
        super().__init__()
...
class FocalTversky_loss(nn.Module):
    ...
    author code:
    https://github.com/nabsabraham/focal-tversky-unet/blob/347d39117c24540400dfe80d106d2fb06d2b99e1/losses.py#L65
    """
